...
The first task of this assignment is to create a function that represents the formula above. I have chosen to use for loops for this.
```{r}
B <- function(n) {
  sum <- 0
  for (k in 0:n) {
...
```
We can then use n = 2 and n = 10 to see if the function works and is correct.
```{r}
B(2)
```
...
B(10)
```
After that, we can evaluate the function.
```{r}
microbenchmark::microbenchmark(B(2), B(10))
```

# Task B
The second task of this assignment is to create a function that compares two beta estimates from two different packages, mgcv and gam. The following function is written but does not work when the ragweed data is put in.
```{r, message=FALSE, warning=FALSE}
# Open necessary packages
library(gam)
library(mgcv)
# Create function that takes as inputs data, response variable, 
...
Semifun <- function(data, response_variable, target_variable, explanatory_variable) {
  ...
  names(final_vector) <- c("mgcv beta fit", "gam beta fit", "number of observations", 
                           "number of explanatory variables")
  return(final_vector)
}
```
After this, we try the function out using the ragweed data.
...
```
Since this code produces an error message that I am not able to repair, I do not consider this task to be completed.

# Task C
The final task of this assignment is to recreate a figure that consists of 3 plots using the CO2 data. The first thing we will do is to calculate the mean using the aggregate function. After that, a variable called test is created to make sure that the lines in the first plot is separated by type and treatment. 
```{r}
# Load data
data("CO2")
...
```
In order to recreate the plots, I will use the ggplot2 package. 
```{r, warning=FALSE}
library(ggplot2)
```
All that remains is to recreate the plots.
```{r, warning=FALSE}
# Line plot
...
```
The figure is not exactly the one given in the ssignment description, but it's somewhat close.